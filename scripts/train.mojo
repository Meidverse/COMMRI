"""
MRI Classification Training Script (Standalone)
=================================================
Training script with tqdm progress bars.
"""

from python import Python


fn main() raises:
    print("=" * 60)
    print("MRI Classification System - Mojo Training")
    print("=" * 60)
    
    var np = Python.import_module("numpy")
    var builtins = Python.import_module("builtins")
    var tqdm_module = Python.import_module("tqdm")
    var tqdm = tqdm_module.tqdm
    
    # Configuration
    var num_epochs = 50
    var batch_size = 4
    var num_batches = 20
    var num_classes = 2
    var input_size = 32
    var learning_rate = 0.001
    
    print("\nConfiguration:")
    print("  Epochs:", num_epochs)
    print("  Batch size:", batch_size)
    print("  Input size:", input_size, "^3")
    print("  Num classes:", num_classes)
    print("  Learning rate:", learning_rate)
    
    # Initialize weights
    var input_features = 1 * input_size * input_size * input_size
    print("  Input features:", input_features)
    
    np.random.seed(42)
    var weights_W = np.random.randn(input_features, num_classes) * 0.01
    var weights_b = np.zeros(num_classes)
    
    print("\nStarting training...\n")
    
    var best_loss = 1000000.0
    var history_loss = builtins.list()
    var history_acc = builtins.list()
    
    # Epoch progress bar
    var epoch_pbar = tqdm(builtins.range(num_epochs), desc="Training", unit="epoch")
    
    for epoch in epoch_pbar:
        var epoch_int = Int(epoch)
        var epoch_loss = 0.0
        var epoch_acc = 0.0
        
        for batch_idx in range(num_batches):
            var seed = epoch_int * num_batches + batch_idx
            np.random.seed(seed)
            
            # Create synthetic data
            var x = np.random.randn(batch_size, 1, input_size, input_size, input_size)
            var shape = builtins.tuple([batch_size, num_classes])
            var y = np.zeros(shape)
            for b in range(batch_size):
                var class_idx = (seed + b) % num_classes
                y[b][class_idx] = 1.0
            
            # Forward pass
            var flat_x = x.reshape(batch_size, -1)
            var logits = np.matmul(flat_x, weights_W) + weights_b
            
            # Softmax
            var exp_logits = np.exp(logits - np.max(logits, axis=1, keepdims=True))
            var pred = exp_logits / np.sum(exp_logits, axis=1, keepdims=True)
            
            # Loss
            var eps = 1e-7
            var pred_clipped = np.clip(pred, eps, 1.0 - eps)
            var loss_val = -np.sum(y * np.log(pred_clipped)) / batch_size
            var loss = Float64(loss_val)
            
            # Accuracy
            var pred_classes = np.argmax(pred, axis=1)
            var true_classes = np.argmax(y, axis=1)
            var correct = np.sum(pred_classes == true_classes)
            var acc = Float64(correct) / Float64(batch_size)
            
            epoch_loss += loss
            epoch_acc += acc
            
            # Gradient descent
            var grad_pred = (pred - y) / batch_size
            var grad_W = np.matmul(flat_x.T, grad_pred)
            var grad_b = np.sum(grad_pred, axis=0)
            
            weights_W = weights_W - learning_rate * grad_W
            weights_b = weights_b - learning_rate * grad_b
        
        # Average metrics
        var avg_loss = epoch_loss / Float64(num_batches)
        var avg_acc = epoch_acc / Float64(num_batches)
        
        _ = history_loss.append(avg_loss)
        _ = history_acc.append(avg_acc)
        
        if avg_loss < best_loss:
            best_loss = avg_loss
        
        # Update progress bar
        _ = epoch_pbar.set_postfix(loss=avg_loss, acc=avg_acc, best=best_loss)
    
    print("\n" + "=" * 60)
    print("Training Complete!")
    print("=" * 60)
    print("Best Loss:", best_loss)
    print("Final Accuracy:", Float64(history_acc[-1]))
    print("=" * 60)
